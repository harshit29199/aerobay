...
class planedummy extends StatelessWidget {
  planedummy({
...
class _dummy1State extends State<dummy1> {
  ...
  void savedDevice() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    preferences.setString('plane', widget.characteristic.deviceId);
  }
...
// Here you can write your code
  ...
  void _updateJoystickPosition(Offset details, Size size) {
    ...
    double verticalValue = ((1 - normalizedY) * 50).clamp(0, 100);
  ...
  void _printDirectionAndValue(double horizontalValue, double verticalValue) {
    if (horizontalValue > 0) {
      ...
      String command = 'R${horizontalValue.toStringAsFixed(0)}';
      sendFun1(command.codeUnits);
    } else if (horizontalValue < 0) {
      print('L${horizontalValue.toStringAsFixed(0)}'); // Left direction
      String command = 'L${horizontalValue.toStringAsFixed(0)}';
      sendFun1(command.codeUnits);
    } else if (verticalValue >= 0) {
      print('F${verticalValue.toStringAsFixed(0)}'); // Down direction
      String command = 'F${verticalValue.toStringAsFixed(0)}';
      sendFun1(command.codeUnits);
    }
    // else if (verticalValue < 0) {
    //   print('F${verticalValue.toStringAsFixed(0)}'); // Up direction
    //   String command = 'F${verticalValue.toStringAsFixed(0)}';
    //   sendFun1(command.codeUnits);
    // }
    else {
  ...
  void _updateJoystickPosition1(Offset details, Size size) {
    ...
    // Map vertical range to -90 (bottom) to 90 (top)
    double horizontalValue = (normalizedX * 90).clamp(-90, 90);
    double verticalValue = (-normalizedY * 90).clamp(-90, 90); // Inverted mapping
  ...
  void _printMovementDirection(double horizontalValue, double verticalValue) {
    if (horizontalValue > 0) {
      ...
      String command = 'T${verticalValue.toStringAsFixed(0)}';
      sendFun1(command.codeUnits);
    } else if (verticalValue < 0) {
      print('B${verticalValue.toStringAsFixed(0)}'); // Down direction
      String command = 'B${verticalValue.toStringAsFixed(0)}';
      sendFun1(command.codeUnits);
    } else {
